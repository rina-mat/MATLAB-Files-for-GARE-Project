function Combination_General_Polygon_Meshing_Program_15_12
...
maxref = input('Enter Number of mesh refinements (Put any integer value (1 to 4 is preferable)): ');
...
if oe == 1
    [nodeCord  QuadNode] = meshConv1(quad, node, ne, n);
...
np = input('');
fprintf('Maximum Number of Vertices in a Subdomain? \n');
max = input('');
part = zeros(np,max)
fprintf('Enter sub-domain elements: \n');
total = 0;
for i=1:np
    count(i) = 0;
    fprintf('Part[%d]: ',i);
...
if tp == 0
fprintf('Enter Number of Elements: %i or %i or %i or %i or %i\n', np, total, 4*total, 4*4*total, 4*4*4*total);
else
  fprintf('Enter Number of Elements: %i or %i or %i or %i\n', total, 4*total, 4*4*total, 4*4*4*total);  
end
ne = input('Elements? ');
...
return;
 ...
 function [node quad] = meshConv1(quad, node, ne, n)
